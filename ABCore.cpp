...
int16_t ABCore::alpha_beta(PositionState& pos, uint16_t depth, int16_t alpha, int16_t beta, bool white_to_play)
{
	...
	if (depth == 0) {
		int16_t score = quiescence_search(pos, alpha, beta, white_to_play);
		eval_info eval(score, pos.get_zob_key(), depth);
...
int16_t ABCore::quiescence_search(PositionState& pos, int16_t alpha, int16_t beta, bool white_to_play)
{
	...
	if (white_to_play) {	
		...
		for (uint16_t i = 0; i < possible_moves.size(); ++i) {
			pos.make_move(possible_moves[i]);
			score = quiescence_search(pos, current_alpha, current_beta, !white_to_play);
			pos.undo_move();
			if (score > current_alpha) {
				current_alpha = score;
			}
